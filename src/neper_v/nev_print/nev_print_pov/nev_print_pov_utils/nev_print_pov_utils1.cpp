...
#include"nev_print_pov_utils_.h"

void
nev_print_pov_segment (FILE * file, double *coo1,
		   double *coo2, char *edge_rad, char *edge_texture)
{
  if (!nev_print_pov_cylinder_test (coo1, coo2, edge_rad))
    return;
...
void
nev_print_pov_cone (FILE * file, double *coo1,
		double *coo2, char *edge_rad, char *edge_texture)
{
  if (!nev_print_pov_cylinder_test (coo1, coo2, edge_rad))
    return;
...
void
nev_print_pov_segment_wsph (FILE * file, double *coo1,
			double *coo2, char *edge_rad, char *edge_texture)
{
  if (!nev_print_pov_cylinder_test (coo1, coo2, edge_rad))
    return;
...
void
nev_print_pov_sphere (FILE * file, double *coo, char *sphere_rad, char *texture)
{
  double rad;
...
void
nev_print_pov_ellipsoid (FILE * file, double *coo, double *rad, char *texture)
{
  fprintf (file,
...
void
nev_print_pov_cube (FILE * file, double *coo, double *rad, char *texture)
{
  fprintf (file,
...
void
nev_print_pov_cyl (FILE * file, double *coo, double *rad, char *texture)
{
...
void
nev_print_pov_triangle_edge (FILE * file, double *coo1, double *coo2,
...
void
nev_print_pov_mesh2d (FILE * file, struct NODES Nodes, struct MESH Mesh,
...
void
nev_print_pov_triangle (FILE * file, double *coo1, double *coo2, double *coo3,
		    char *texture, char *edge_rad, char *edge_texture)
{
  ...
  nev_print_pov_triangle_edge (file, coo1, coo2, coo3, edge_rad, edge_texture);

  return;
}

void
nev_print_pov_tet_edge (FILE * file, double *coo1, double *coo2, double *coo3,
...
void
nev_print_pov_polygon_edge (FILE * file, int qty, double **coo,
...
void
nev_print_pov_polygon (FILE * file, int qty, double **coo,
		   char *texture, char *edge_rad, char *edge_texture,
		   int pt, double *p, char *iedge_rad, char *iedge_texture)
{
  ...
  for (i = 0; i < qty; i++)
  {
    ...
    if (p1 != pt && p2 != pt)
      nev_print_pov_triangle (file, c, coo[p1], coo[p2], texture, NULL, NULL);
  }

  nev_print_pov_polygon_edge (file, qty, coo, edge_rad, edge_texture);

  // drawing segment face interpolation
  if (iedge_rad != NULL)
    for (i = 0; i < qty; i++)
      nev_print_pov_segment (file, c, coo[i], iedge_rad, iedge_texture);
...
void
nev_print_pov_rectangle (FILE * file, double *coo, double *size,
		     char *texture, char *edge_rad, char *edge_texture)
{
  int i;
  ...
  nev_print_pov_polygon (file, 4, coo2, texture, edge_rad, edge_texture,
		     -1, NULL, NULL, NULL);
...
void
nev_print_pov_tet (FILE * file, double *coo1, double *coo2, double *coo3,
	       double *coo4, char *texture, char *edge_rad,
	       char *edge_texture)
{
  nev_print_pov_triangle (file, coo1, coo2, coo3, texture, NULL, NULL);
  nev_print_pov_triangle (file, coo1, coo3, coo4, texture, NULL, NULL);
  nev_print_pov_triangle (file, coo1, coo4, coo2, texture, NULL, NULL);
  nev_print_pov_triangle (file, coo2, coo3, coo4, texture, NULL, NULL);

  nev_print_pov_tet_edge (file, coo1, coo2, coo3, coo4, edge_rad, edge_texture);

  return;
}

void
nev_print_pov_scale (FILE * file, char *ColScheme, char *scale, char *scaletitle)
{
  int i;
  struct NODES N;
  struct MESH M;
  ...
  nev_print_pov_header (file, Print);
  nev_print_pov_mesh2d (file, N, M, NULL, rgb, (char *) "elt", Print.showshadow);
  nev_print_pov_foot (file, Print);
...
void
nev_print_pov_arrow (FILE * file, double *O,
		 double *v, char *edge_rad, char *edge_texture)
{
  ...
  nev_print_pov_segment (file, coo[0], coo[1], edge_rad, edge_texture);
  nev_print_pov_cone (file, coo[1], coo[2], tip_rad, edge_texture);
  nev_print_pov_sphere (file, coo[0], edge_rad, edge_texture);
...
void
nev_print_pov_pov2png (char *povray, char *filename, int imagewidth,
		   int imageheight, int imageantialias, int messagetag)
{
  ...
  if (imageantialias > 0)
    ...
    sprintf (command,
	     "%s Input_File_Name=%s +O%s +W%d +H%d -D 2>/dev/null",
	     povray, filename, outfilename, imagewidth, imageheight);
  ...
  if (system (command) == -1 || !ut_file_exist (outfilename))
  {
    ...
    if (imageantialias > 0)
      ...
      sprintf (command, "%s Input_File_Name=%s +O%s +W%d +H%d -D",
	       povray, filename, outfilename, imagewidth, imageheight);
