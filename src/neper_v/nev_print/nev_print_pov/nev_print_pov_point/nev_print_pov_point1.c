...
#include"nev_print_pov_point_.h"

void
nev_print_pov_point (FILE * file, struct POINT Point, struct POINTDATA PointData,
		 struct PRINT Print)
{
  ...
  for (i = 1; i <= Point.PointQty; i++)
	...
	nev_print_pov_sphere (file, PointData.Coo[i], string, texture);
      }
      else
      {
	double **V = ol_g_alloc ();
	...
	if (!strcmp (PointData.RadDataType, "cube"))
	  nev_print_pov_cube (file, PointData.Coo[i], data, texture);
	else if (!strcmp (PointData.RadDataType, "cyl"))
	  nev_print_pov_cyl (file, PointData.Coo[i], data, texture);
	else if (!strcmp (PointData.RadDataType, "ell"))
	  nev_print_pov_ellipsoid (file, PointData.Coo[i], data, texture);
	else
	  abort ();
