...
#include"nev_print_pov_.h"

void
nev_print_pov (char *body, struct PRINT Print,
...
{
  FILE *file = NULL;
  char *filename = ut_alloc_1d_char (strlen (body) + 100);
  char *filename2 = ut_alloc_1d_char (strlen (body) + 100);
  ...
  if (Print.showcsys == 1)
    nev_print_pov_csys (file, CsysData, Print);

  if (Print.showpoint[0] > 0)
    nev_print_pov_point (file, Point, PointData, Print);

  // tessellation ------------------------------------------------------
  if (Print.showtess == 1)
  {
    ut_print_message (0, 2, "Printing tessellation...\n");
    nev_print_pov_tess (file, Print, Tess, TessData);
  }
  ...
  if (Print.showtesr == 1)
  {
    ...
    nev_print_pov_tesr (file, Print, Tesr, TesrData);
  }
  ...
  if (Print.showmesh == 1)
  {
    ...
    nev_print_pov_mesh (file, Print, Tess, Nodes, Mesh, NodeData, MeshData);
  }
  ...
  if (SQty > 0 && Print.showslice)
  {
	...
	nev_print_pov_mesh2d (file, SNodes[i], SMesh2D[i], showelt2d,
  ...
  if (ut_string_inlist (Print.format, NEUT_SEP_NODEP, "png"))
  {
    ...
    nev_print_pov_pov2png (Print.povray, filename, imagewidth,
  ...
  for (i = 0; i <= 3; i++)
	...
	&& !strcmp (MeshData[i].ColDataType, "scal"))
    {
      ...
      sprintf (filename2, "%s-scale%s.pov", body, ent);

      file = ut_file_open (filename2, "w");
      nev_print_pov_scale (file, MeshData[i].ColScheme, MeshData[i].Scale,
		       MeshData[i].ScaleTitle);
      ut_file_close (file, filename2, "w");

      if (ut_string_inlist (Print.format, NEUT_SEP_NODEP, "png"))
	nev_print_pov_pov2png (Print.povray, filename2, 0.3 * imageheight,
  ...
  if (NodeData.ColDataType != NULL && !strcmp (NodeData.ColDataType, "scal"))
  {
    ...
    sprintf (filename2, "%s-scalen.pov", body);

    file = ut_file_open (filename2, "w");
    nev_print_pov_scale (file, NodeData.ColScheme, NodeData.Scale,
		     NodeData.ScaleTitle);
    ut_file_close (file, filename2, "w");
  ...
  for (i = 0; i <= 3; i++)
	...
	&& !strcmp (TessData.ColDataType[i], "scal"))
    {
      ...
      sprintf (filename2, "%s-scale%d.pov", body, i);

      file = ut_file_open (filename2, "w");
      nev_print_pov_scale (file, TessData.ColScheme[i], TessData.Scale[i],
      ...
      && !strcmp (TesrData.ColDataType, "scal"))
  {
    ...
    sprintf (filename2, "%s-scale.pov", body);

    file = ut_file_open (filename2, "w");
    nev_print_pov_scale (file, TesrData.ColScheme, TesrData.Scale,
		     TesrData.ScaleTitle);
    ut_file_close (file, filename2, "w");
      ...
      && !strcmp (PointData.ColDataType, "scal"))
  {
    ...
    sprintf (filename2, "%s-scalep.pov", body);

    file = ut_file_open (filename2, "w");
    nev_print_pov_scale (file, PointData.ColScheme, PointData.Scale,
