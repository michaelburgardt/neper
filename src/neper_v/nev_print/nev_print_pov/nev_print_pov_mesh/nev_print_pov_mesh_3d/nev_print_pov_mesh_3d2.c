...
#include"nev_print_pov_mesh_3d_.h"

void
nev_print_pov_mesh_3d_compress (struct PRINT Print, struct NODES Nodes,
			    struct MESH Mesh3D,
			    struct NODES *pN,
			    struct MESH *pM2D,
			    int ***pprintelt3dface,
...
{
  nev_print_pov_mesh_3d_print_faces (file, Print, N, M2D,
				 elt2delt3d, nodes_new_old,
				 NodeData, MeshData);

  nev_print_pov_mesh_3d_print_edges (file, Print.showshadow, N, M1D, MeshData);
