...
#include"nev_print_pov_mesh_2d_.h"

void
nev_print_pov_mesh_2d_print_faces (FILE * file, struct PRINT Print,
			       ...
			       struct MESHDATA *MeshData)
{
  ...
  if (!strncmp (coldatatype, "elt", 3))
  {
    ...
    nev_print_pov_mesh2d (file, N, M2D, Print.showelt2d, rgb, "elt",
		      Print.showshadow);
  ...
  else
  {
    ...
    nev_print_pov_mesh2d (file, N, M2D, Print.showelt2d, rgb, "node",
		      Print.showshadow);
...
void
nev_print_pov_mesh_2d_print_edges (FILE * file, int showshadow,
			       ...
			       struct MESHDATA *MeshData)
{
  ...
  for (i = 1; i <= M1D.EltQty; i++)
    nev_print_pov_segment_wsph (file, N.NodeCoo[M1D.EltNodes[i][0]],
