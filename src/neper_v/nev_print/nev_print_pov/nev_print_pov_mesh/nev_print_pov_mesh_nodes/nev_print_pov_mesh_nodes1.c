...
#include"nev_print_pov_mesh_nodes_.h"

void
nev_print_pov_mesh_nodes (FILE * file, struct PRINT Print, struct NODES
		      Nodes, struct MESH *Mesh, struct NODEDATA NodeData)
{
  ...
  for (i = 1; i <= Nodes.NodeQty; i++)
    if (Print.shownode[i])
    {
      ...
      nev_print_pov_sphere (file, NodeData.Coo[i], string, texture);
