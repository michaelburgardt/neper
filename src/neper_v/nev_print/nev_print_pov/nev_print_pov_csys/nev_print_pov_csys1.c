...
#include"nev_print_pov_csys_.h"

void
nev_print_pov_csys (FILE * file, struct CSYSDATA CsysData, struct PRINT Print)
{
  int i;
  ...
  fprintf (file,
	   ...
	   CsysData.Col[0] / 255., CsysData.Col[1] / 255.,
	   CsysData.Col[2] / 255.);

  sprintf (edge_rad, "%f", CsysData.Rad);
  sprintf (edge_texture, "csys");

  ut_array_2d_scale (g, 3, 3, CsysData.Length);
  ut_array_2d_scale (s, 3, 3, 0.1);

  ut_array_1d_memcpy (O, 3, CsysData.Coo);

  for (i = 0; i < 3; i++)
    if (strlen (CsysData.Label[i]) > 0)
    {
      nev_print_pov_arrow (file, O, g[i], edge_rad, edge_texture);

      fprintf (file, "#declare CamLoc = <%f,%f,%f>;\n",
	       Print.cameracoo[0], Print.cameracoo[1], Print.cameracoo[2]);
      fprintf (file, "#declare CamSky = <%f,%f,%f>;\n",
	       Print.camerasky[0], Print.camerasky[1], Print.camerasky[2]);
      fprintf (file, "#declare CamLA = <%f,%f,%f>;\n",
	       Print.cameralookat[0], Print.cameralookat[1],
	       Print.cameralookat[2]);

      fprintf (file, "#declare TextLowerLeft=<%f, %f, %f>;\n",
	       O[0] + g[i][0], O[1] + g[i][1], O[2] + g[i][2]);
