...
void
nev_print_pov_tess_pre (struct PRINT Print, struct TESS Tess,
...
void
nev_print_pov_tess_polyfaces (FILE *file, struct PRINT Print, struct TESS Tess,
			  struct TESSDATA TessData, int *polyfacelist)
{
  ...
  if (Tess.Dim == 3)
	...
	ut_array_1d_memcpy (coo[j - 1], 3,
      ...
      nev_print_pov_polygon (file, Tess.FaceVerQty[face], coo, texture,
...
void
nev_print_pov_tess_faces (FILE *file, struct PRINT Print, struct TESS Tess,
                      struct TESSDATA TessData, int *facelist)
{
  ...
  for (i = 1; i <= facelist[0]; i++)
  {
	...
	faceinter_trs += TessData.trs[1][Tess.FaceEdgeNb[face][j]];
      ...
      {
	if (Tess.FacePt[face] <= 0)
	  nev_print_pov_segment (file, p, coo[j], "faceinter_rad",
			     "faceinter_texture");
...
void
nev_print_pov_tess_edges (FILE *file, struct PRINT Print, struct TESS Tess,
                      struct TESSDATA TessData, int *edgelist)
{
  ...
  for (i = 1; i <= edgelist[0]; i++)
  {
    ...
    nev_print_pov_segment_wsph (file, coo[0], coo[1], string, texture);
...
void
nev_print_pov_tess_vers (FILE *file, struct PRINT Print, struct TESS Tess,
                     struct TESSDATA TessData, int *verlist)
{
  ...
  for (i = 1; i <= verlist[0]; i++)
  {
    ...
    nev_print_pov_sphere (file, Tess.VerCoo[ver], string, texture);
    ut_free_1d_char (string);
  }
...
void
nev_print_pov_tess_seeds (FILE *file, struct PRINT Print, struct TESS Tess,
                      struct TESSDATA TessData, int *seedlist)
{
  ...
  for (i = 1; i <= seedlist[0]; i++)
  {
    ...
    nev_print_pov_sphere (file, Tess.SeedCoo[seed], string, texture);
    ut_free_1d_char (string);
  }
