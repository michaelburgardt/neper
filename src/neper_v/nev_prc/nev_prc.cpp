           ...
           struct POINT Point, struct POINTDATA PointData)
{
  ...
  if (Print.showtess == 1)
  {
    ...
    for (int ver = 1; ver <= Tess.VerQty; ver++)
    ...
    for (int edge = 1; edge <= Tess.EdgeQty; edge++) {
      if (Tess.EdgeState[edge] != -1) {
        double points[2][3];
        points[0][0] = Tess.VerCoo[Tess.EdgeVerNb[edge][0]][0];
        points[0][1] = Tess.VerCoo[Tess.EdgeVerNb[edge][0]][1];
        points[0][2] = Tess.VerCoo[Tess.EdgeVerNb[edge][0]][2];
        points[1][0] = Tess.VerCoo[Tess.EdgeVerNb[edge][1]][0];
    ...
    for (int poly=1; poly<= Tess.PolyQty; poly++) {
      ...
      for (int face=1; face<= Tess.PolyFaceQty[poly]; face++) {
        for(int ver=1; ver+2<= Tess.FaceVerQty[ Tess.PolyFaceNb[poly][face] ]; ver++) {
          double points[3][3];
          points[0][0] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][1] ][0];
          points[0][1] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][1] ][1];
          points[0][2] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][1] ][2];
          points[1][0] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][ver+1] ][0];
          points[1][1] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][ver+1] ][1];
          points[1][2] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][ver+1] ][2];
          points[2][0] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][ver+2] ][0];
          points[2][1] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][ver+2] ][1];
          points[2][2] = Tess.VerCoo[ Tess.FaceVerNb[ Tess.PolyFaceNb[poly][face] ][ver+2] ][2];
...
*/
    ...
    for (int node = 1; node <= N.NodeQty; node++)
         file.addPoint(N.NodeCoo[node], prc::RGBAColour(0.0,0.0,0.0));

    for (int edge = 1; edge <= M1D.EltQty; edge++) {
      double points[2][3];
      points[0][0] = N.NodeCoo[M1D.EltNodes[edge][0]][0];
      points[0][1] = N.NodeCoo[M1D.EltNodes[edge][0]][1];
      points[0][2] = N.NodeCoo[M1D.EltNodes[edge][0]][2];
      points[1][0] = N.NodeCoo[M1D.EltNodes[edge][1]][0];
    ...
    prc::PRCmaterial materialGrain(prc::RGBAColour(     Color[0],      Color[1],      Color[2], 1),  // Diffuse
                                   ...
                                   prc::RGBAColour( 0.2*Color[0],  0.2*Color[1],  0.2*Color[2], 1),  // Emissive
                                   prc::RGBAColour(0.05*Color[0], 0.05*Color[1], 0.05*Color[2], 1),  // Ambiant
                                   1. - TessData.trs[3][1] /* transparency */,  0.7 /* Shininess */);
